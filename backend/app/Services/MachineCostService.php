...
class MachineCostService
{
    ...
    public function period(Machine $machine, string $from, string $to): array
    {
        ...
        $repls = ComponentReplacement::where('account_id', $machine->account_id)->whereHas('newLifecycle.machineComponent', fn ($q) => $q->where('machine_id', $machine->id))->where('replaced_at', '>=', $start)->where('replaced_at', '<', $end)->get();
        ...
        $clicks = null;
        if ($type) {
            $rows = CounterReading::where('account_id', $machine->account_id)->where('machine_id', $machine->id)->where('counter_type_id', $type->id)->where('status', 'effective')->where('observed_at', '>=', $start)->where('observed_at', '<', $end)->orderBy('observed_at')->get();
        ...
        return ['machine_id' => $machine->id, 'resolved_timezone' => $this->tz->resolve($machine), 'period_start' => $from, 'period_end' => $to, 'period_clicks' => $clicks, 'known_consumption_cost' => number_format((float) $known, 2, '.', ''), 'unknown_consumption_events' => $unknown, 'error_waste_cost' => number_format((float) $loss, 2, '.', ''), 'standard_machine_cost' => number_format($standard, 2, '.', ''), 'machine_cost_per_click' => $clicks > 0 ? number_format($standard / $clicks,4,'.','') : null, 'economics_status' => $unknown ? 'PARTIAL' : 'COMPLETE'];
    }
}
