...
class InventoryController extends Controller
{
    ...
    public function locations(Request $r)
    {
        ...
        return response()->json(['data' => InventoryLocation::whereIn('account_id', $ids)->where('is_active', true)->orderBy('name')->get()]);
    }

    public function createPurchase(Request $r)
    {
        $d = $r->validate(['account_id' => 'required|uuid', 'purchase_number' => 'required|string', 'purchase_date' => 'required|date', 'currency_code' => 'nullable|string|size:3', 'notes' => 'nullable|string', 'client_request_id' => 'required|uuid', 'lines' => 'required|array|min:1']);
    ...
    public function transfer(Request $r)
    {
        ...
        return response()->json(['data' => app(InventoryLedgerService::class)->transfer($item, InventoryLocation::findOrFail($d['from_location_id']), InventoryLocation::findOrFail($d['to_location_id']), $d['quantity'], $d['client_request_id'])], 201);
    }
    ...
    {
        $d = $r->validate(['inventory_source' => 'required|in:inventory,external_untracked', 'inventory_item_id' => 'nullable|uuid', 'inventory_location_id' => 'nullable|uuid', 'quantity' => 'nullable|numeric|min:0.0001', 'replaced_at' => 'nullable|date', 'external_reason' => 'nullable|string', 'notes' => 'nullable|string', 'client_request_id' => 'required|uuid']);
