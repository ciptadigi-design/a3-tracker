...
class MaintenanceDocumentExtractionTest extends TestCase
{
    ...
    // Maintenance Clean Slate Phase 1: the HTTP entry points this file used to test here
    // (POST .../extract, GET .../extraction, GET .../pages) were retired from routing -
    // see routes/api.php and MaintenanceDocumentStorageTest for the "upload does not
    // dispatch extraction" regression. ExtractMaintenanceDocumentJob itself remains in
    // the codebase, unreachable, and is still covered below at the job level.
    ...
    public function test_error_response_never_exposes_a_filesystem_path_or_stack_trace(): void
    {
        ...
        $this->assertSame('FAILED', $extraction->status);
        $this->assertSame('UNSUPPORTED_PDF_SECURITY', $extraction->error_code);
        $this->assertStringNotContainsString(storage_path(), $extraction->error_message);
        $this->assertStringNotContainsString('.php', $extraction->error_message);
        $this->assertStringNotContainsString('Stack trace', $extraction->error_message);
    }
    ...
    // Maintenance Clean Slate Phase 1: the GET .../pages HTTP endpoint this file used to
    // test here was retired from routing - see routes/api.php.
