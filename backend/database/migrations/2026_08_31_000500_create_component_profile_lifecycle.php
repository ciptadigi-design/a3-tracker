...
return new class extends Migration {
    public function up(): void {
        ...
        Schema::create('machine_component_exclusions', function (Blueprint $t) {
            ...
            $t->foreign('account_id')->references('id')->on('accounts')->restrictOnDelete(); $t->foreign('machine_id')->references('id')->on('machines')->restrictOnDelete(); $t->foreign('profile_slot_id')->references('id')->on('model_profile_slots')->restrictOnDelete(); $t->unique(['machine_id','profile_slot_id','cleared_at'], 'mc_excl_machine_slot_clear_uq'); $t->index(['machine_id','profile_slot_id','cleared_at'], 'mc_excl_scope_idx');
        });
        Schema::create('component_lifecycles', function (Blueprint $t) {
            ...
            $t->foreign('machine_component_id')->references('id')->on('machine_components')->restrictOnDelete(); $t->unique(['machine_component_id','active_key'], 'component_lifecycle_active_uq'); $t->unique(['client_request_id'], 'component_lifecycle_request_uq'); $t->index(['machine_component_id','started_at'], 'component_lifecycle_time_idx');
        });
    }
