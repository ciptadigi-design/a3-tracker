...
const source = JSON.parse(readFileSync(new URL('../../.migration-private/m2-10b/958bb2b3-3110-410f-9c03-d8355a2f9be7/legacy-snapshot.json', import.meta.url))).tables
...
test('Production-like empty target reclassifies DEV merge as deterministic import', () => {
...
test('legacy migration SQL does not create opening stock from legacy snapshot or purchases', async () => {
...
test('Production Account Branch Machine mapping and Graha denylist remain exact', () => {
...
test('archive-only and exclusion evidence is preserved with full source accounting', () => {
  const plan = buildM211Plan(source)
...
test('deterministic plan and migration IDs survive a full re-plan', () => {
