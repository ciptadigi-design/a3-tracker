...
function dispositionMap() {
  return new Map(dispositionArtifact.rows.map((row) => [`${row.legacy_table}:${row.legacy_id}`, row]))
}

export function buildMigrationPlan(source) {
  const disposition = dispositionMap()
  ...
  for (const mapping of picArtifact.mappings.filter((row) => row.treatment === 'CREATE_PERSON')) {
    const id = idFor('person', 'actor', mapping.candidate_target_person)
    createdPeople.set(mapping.candidate_target_person, id)
    operations.people.push({ id, name: mapping.candidate_target_person })
  }

  const targetCounterEvents = [
  ...
  const counterImports = source.click_history.map((row) => ({ row, d: disposition.get(`click_history:${row.id}`), decision: counterDecisions.get(row.id) }))
    ...
    .map(({ row, d, decision }) => ({ id: idFor('row', 'click_history', row.id), sourceId: row.id, value: n(row.total_clicks), observed_at: decision.migration_timestamp, evidence: decision.timestamp_evidence, operator: resolvePic(row.operator), rawOperator: row.operator, disposition: d.disposition }))
  ...
  for (const row of source.click_history) {
    ...
    crosswalk.push(crosswalkRow('click_history', row.id, d, targetId ? 'counter_readings' : null, targetId, decision?.timestamp_evidence))
  }
  ...
  for (const rows of Object.values(replacementGroups)) {
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (d.disposition === 'IMPORT') {
        ...
        operations.lifecycles.push({ id, sourceId: row.id, slot, profileId: profileId(slot), componentId: componentId(slot), installed: n(previous.replaced_at_click), removed: n(row.replaced_at_click), startSourceId: previous.id, operator: row.operator, createdAt: row.created_at })
        crosswalk.push(crosswalkRow('part_replacements', row.id, d, 'machine_component_lifecycles', id, `LEVEL_A interval starts at ${previous.id}`))
      } else crosswalk.push(crosswalkRow('part_replacements', row.id, d, null, null, d.reason))
    }
  }
  ...
  for (const row of source.part_purchases) {
    ...
    if (!suppliers.has(supplierKey)) suppliers.set(supplierKey, { id: idFor('supplier', 'supplier', supplierKey), name: supplierName, code: `LEG-${deterministicFingerprint(supplierKey).slice(0, 10).toUpperCase()}` })
    if (!items.has(itemKey)) items.set(itemKey, { id: idFor('item', 'inventory_item', itemKey), name: row.part_name, sku: `LEG-${deterministicFingerprint(itemKey).slice(0, 12).toUpperCase()}`, componentId: slot ? componentId(slot) : null })
  ...
  for (const row of source.error_logs) {
    ...
    if (d.disposition === 'IMPORT') {
      const person = resolvePic(row.pic); const base = n(row.kerugian_bahan) + n(row.kerugian_jasa); const stored = n(row.jumlah_kerugian); const multiplier = base > 0 ? stored / base : 1
      ...
      crosswalk.push(crosswalkRow('error_logs', row.id, d, 'operational_incidents', id, multiplier > 1 ? `preserved source multiplier ${multiplier}` : null))
    } else crosswalk.push(crosswalkRow('error_logs', row.id, d, null, null, d.reason))
  }
  for (const row of source.inventory_parts) { const d = disposition.get(`inventory_parts:${row.id}`); crosswalk.push(crosswalkRow('inventory_parts', row.id, d, null, null, 'historical display snapshot; no opening stock')) }
  crosswalk.sort((a,b) => a.source_table.localeCompare(b.source_table) || String(a.source_id).localeCompare(String(b.source_id)))
  const dispositionTotals = Object.fromEntries(['IMPORT','MERGE','SKIP_DUPLICATE','ARCHIVE_ONLY','MANUAL_REVIEW'].map((key) => [key, crosswalk.filter((row) => row.disposition === key).length]))
  ...
  return { version: 'm2.10a-v1', operations, crosswalk, dispositionTotals, eligibleAccounted: total - dispositionTotals.MANUAL_REVIEW, unexplainedRemainder: 526 - total }
}

function crosswalkRow(table, id, disposition, targetEntity, targetId, notes) {
  return { source_project: IDENTITY.sourceProject, source_schema: 'public', source_table: table, source_id: String(id), target_entity: targetEntity, target_id: targetId, mapping_rule: disposition.eligibility, confidence: disposition.disposition === 'MANUAL_REVIEW' ? 'LOW' : 'HIGH', disposition: disposition.disposition, notes: notes ?? disposition.reason }
}

export function buildApplySql(plan, { dryRun = false, injectFailure = false } = {}) {
  const o = plan.operations; const sql = ['\\set ON_ERROR_STOP on', 'begin;']
  ...
  for (const purchase of o.purchases) {
    sql.push(`insert into public.inventory_purchases(id,account_id,supplier_id,purchase_number,purchase_date,currency_code,status,notes,supplier_code_snapshot,supplier_name_snapshot,client_request_id,created_by,created_by_name_snapshot,updated_by) values(${q(purchase.id)},${q(IDENTITY.accountId)},${q(purchase.supplier.id)},${q(`LEGACY-${purchase.sourceId}`)},${q(purchase.date)},'IDR','draft',${q(`LEGACY_IMPORT; RECEIPT_UNKNOWN_NOT_REPRESENTED; source_id=${purchase.sourceId}`)},${q(purchase.supplier.code)},${q(purchase.supplier.name)},${q(purchase.requestId)},${q(IDENTITY.actorId)},'M2.10A Fixture Owner',${q(IDENTITY.actorId)}) on conflict (id) do nothing;`)
