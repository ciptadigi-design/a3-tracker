...
select extensions.ok((select prosecdef and proconfig @> array['search_path=""']::text[] from pg_proc where oid='public.replace_machine_component(uuid,uuid,uuid,numeric,timestamptz,public.component_replacement_reason,public.component_removal_condition,boolean,uuid,text,text,uuid)'::regprocedure),'replacement RPC is SECURITY DEFINER with empty search path');
select extensions.ok(not has_function_privilege('PUBLIC','public.replace_machine_component(uuid,uuid,uuid,numeric,timestamptz,public.component_replacement_reason,public.component_removal_condition,boolean,uuid,text,text,uuid)','EXECUTE') and not has_function_privilege('anon','public.replace_machine_component(uuid,uuid,uuid,numeric,timestamptz,public.component_replacement_reason,public.component_removal_condition,boolean,uuid,text,text,uuid)','EXECUTE'),'PUBLIC and anonymous RPC execute denied');
...
select extensions.lives_ok($$select public.replace_machine_component('91000000-0000-0000-0000-000000000001','94000000-0000-0000-0000-000000000001','96000000-0000-0000-0000-000000000001',1000000,'2026-02-01','normal_eol','worn',null,'90000000-0000-0000-0000-000000000001','Owner M23C','Normal replacement','97000000-0000-0000-0000-000000000011')$$,'idempotent retry returns existing event');
