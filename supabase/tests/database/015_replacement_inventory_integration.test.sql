...
select extensions.ok(position('machine_record' in lower(pg_get_functiondef('public.replace_machine_component_m27c_base(uuid,uuid,uuid,numeric,timestamptz,public.component_replacement_reason,public.component_removal_condition,boolean,uuid,text,text,uuid,public.component_replacement_inventory_source,uuid,uuid,numeric,text)'::regprocedure)))>0 and position('for update' in lower(pg_get_functiondef('public.replace_machine_component_m27c_base(uuid,uuid,uuid,numeric,timestamptz,public.component_replacement_reason,public.component_removal_condition,boolean,uuid,text,text,uuid,public.component_replacement_inventory_source,uuid,uuid,numeric,text)'::regprocedure)))>0,'guarded replacement implementation retains explicit row-locking strategy');
