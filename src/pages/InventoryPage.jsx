...
export function InventoryPage() {
  ...
  // M2.17.4.1: Supplier Master follows the same branch-eligibility projection as the
  // purchase picker (data.suppliers), so both re-fetch whenever the active branch
  // changes. `allAccountSuppliers` is a separate, lazily-loaded, unfiltered list used
  // only by the "attach existing supplier" admin workflow (see AttachExistingSupplier),
  // never rendered as the normal Supplier Master list.
  const [accountSuppliers, setAccountSuppliers] = useState([])
  const [allAccountSuppliers, setAllAccountSuppliers] = useState(null)
  // M2.17.4.2: refresh() (core stock/movements/locations/purchases) and
  // refreshSuppliers() (the Suppliers panel) are independent requests gated on the
  // same account/branch context, but a transient failure in either one used to set
  // the SAME `error` state - which fatally blocked the whole workspace (Part A) and,
  // even after "Try again" (which only retried refresh()) cleared that shared error,
  // left the supplier list silently stuck empty (Part B's "0 active suppliers"), since
  // refreshSuppliers() was never actually retried. Each request now owns its own error
  // state, and any manual retry re-attempts both.
  ...
  useEffect(() => { setAllAccountSuppliers(null) }, [branch?.id])
