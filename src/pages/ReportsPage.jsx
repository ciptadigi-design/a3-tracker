...
function ReportList({ headers, rows, renderRow, emptyTitle, emptyDetail, resetKey }) {
  const pagination = usePagination(rows.length, resetKey)
  ...
  return <div className="report-table-wrap glass-surface"><div className="report-table-head" style={{ '--report-columns': headers.length }}>{headers.map((header) => <span key={header}>{header}</span>)}</div><div className="report-table-body">{visibleRows.map(renderRow)}</div><Pagination total={rows.length} {...pagination} onPageChange={pagination.setPage} onPageSizeChange={pagination.setPageSize} label="report evidence" /></div>
}
...
export function ReportsPage() {
  ...
  const paginationResetKey = `${branch?.id}:${filters.tab}:${filters.machineId}:${filters.preset}:${period.start}:${period.end}:${filters.errorCategory}:${filters.errorStatus}:${filters.componentSort ?? ''}`
