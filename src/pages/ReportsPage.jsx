...
import { AlertTriangle, BarChart3, Boxes, CalendarRange, CircleDollarSign, Eye, Gauge, Package, Printer, RefreshCcw, ShoppingCart, TrendingUp } from 'lucide-react'
import { PageHeader } from '../components/ui/PageHeader.jsx'
import { ReportDetailDialog } from '../features/reports/ReportDetailDialog.jsx'
import { priceEvidence, reportStatus, reportTabs, validReportFilters } from '../features/reports/reportModel.js'
...
function ReportKpi({ icon, label, value, hint, tone = 'blue' }) {
  return <article className="report-kpi glass-surface"><span className={`report-kpi-icon tone-${tone}`}>{createElement(icon, { size: 19 })}</span><div><span>{label}</span><strong>{value}</strong><small>{hint}</small></div></article>
}
...
export function ReportsPage() {
  ...
  const { value: filters, setUIState: setFilters } = usePersistentUIState({
    uiStateKey: stateKey,
    initialValue: { tab: 'overview', branchId: '', machineId: '', preset: 'this_month', customStart: '', customEnd: '', errorCategory: '', errorStatus: '' },
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      setReport(await loadOperationalReport({ accountId: account.id, branchId: filters.branchId, machineId: filters.machineId,
        periodStart: period.start, periodEnd: period.end, errorCategory: filters.errorCategory, errorStatus: filters.errorStatus }))
    } catch (loadError) { setError(loadError); setReport(null) } finally { setLoading(false) }
  }, [account.id, filters.branchId, filters.errorCategory, filters.errorStatus, filters.machineId, period.end, period.start, validPeriod])
    ...
    {loading ? <div className="report-loading glass-surface"><RefreshCcw className="spin" size={23} /><strong>Building report projections…</strong><span>Reading authoritative operational evidence for the selected range.</span></div> : !report ? <EmptyReport title="Report unavailable" detail="Choose a valid report range and try again." /> : <>
      {filters.tab === 'overview' && <>
        ...
        <section className="report-kpi-grid">
          <ReportKpi icon={Gauge} label="Total Clicks" value={count(overview?.total_clicks)} hint={`${overview?.active_machines ?? 0} active machine projection${overview?.active_machines === 1 ? '' : 's'}`} />
          <ReportKpi icon={Boxes} label="Component Consumption" value={money(overview?.component_consumption_cost)} hint="Known FIFO-backed consumption; unknown evidence remains flagged" tone="purple" />
          <ReportKpi icon={AlertTriangle} label="Error / Waste" value={money(incidentTotal)} hint={`${money(overview?.machine_attributed_error_waste)} machine · ${money(overview?.branch_only_error_waste)} branch-only`} tone="warning" />
          <ReportKpi icon={BarChart3} label="Standard Cost / Click" value={money(overview?.standard_cost_per_click)} hint="Aggregate Standard Machine Cost ÷ aggregate clicks" tone="green" />
          <ReportKpi icon={TrendingUp} label="Estimated Machine Revenue" value={money(overview?.estimated_machine_revenue)} hint="Utilization revenue from historically priced clicks; not invoice revenue" tone="green" />
          <ReportKpi icon={CircleDollarSign} label="Estimated Contribution" value={money(overview?.estimated_standard_contribution)} hint={`Margin ${percent(overview?.contribution_margin_percent)} · not net profit`} tone="blue" />
        </section>
      ...
      {filters.tab === 'components' && <>
        <div className="report-section-heading"><div><span className="card-kicker">Component Consumption</span><h2>Replacement Cost Evidence</h2><p>Logical component identities and CMYK channels remain distinct.</p></div></div>
        <RankingChart title="Known Consumed Cost Ranking" description="FIFO-backed known consumed component cost; unknown events are shown separately." rows={report.components.slice(0, 10).map((row) => ({ ...row, key: `${row.component_id}-${row.machine_id}` }))} valueKey="known_consumed_cost" label={(row) => `${row.component_code} · ${row.machine_code}`} tone="purple" />
      ...
      {filters.tab === 'inventory' && <>
        ...
        <section className="report-kpi-grid compact"><ReportKpi icon={ShoppingCart} label="Purchases" value={String(report.inventory?.purchases ?? 0)} hint={`${money(report.inventory?.purchase_value)} account purchase context`} /><ReportKpi icon={Package} label="Receipts" value={String(report.inventory?.receipts ?? 0)} hint={`${count(report.inventory?.received_quantity)} units received`} tone="green" /><ReportKpi icon={Boxes} label="Inventory Issues" value={String(report.inventory?.issues ?? 0)} hint={`${report.inventory?.replacement_issues ?? 0} replacement issues`} tone="purple" /><ReportKpi icon={AlertTriangle} label="Stock Attention" value={String(Number(report.inventory?.out_of_stock_items ?? 0) + Number(report.inventory?.low_stock_items ?? 0))} hint={`${report.inventory?.out_of_stock_items ?? 0} out · ${report.inventory?.low_stock_items ?? 0} low`} tone="warning" /></section>
