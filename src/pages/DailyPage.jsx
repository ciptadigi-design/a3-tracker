...
export function DailyPage() {
  ...
  const canCorrect = can('counters.correct')
    ...
    <div className="page-stack daily-page">
      ...
      {!machinesState.isLoading && !selectedMachine ? <section className="daily-no-machine glass-surface"><span className="empty-machine-icon"><Printer size={38} /></span><h2>No active machine in this branch.</h2><p>Daily counter input becomes available after a physical machine is registered and active.</p></section>
        : selectedMachine && <>
          ...
          {can('counters.record') && <CounterEntryCard key={`${user.id}:${selectedMachine.id}`} accountId={account.id} branchId={branch.id} userId={user.id} machine={selectedMachine} timezone={timezone} people={counterPeople} peopleLoading={peopleState.isLoading} peopleError={peopleState.error} lastReading={summary.lastReading} onRecorded={handleRecorded} />}
