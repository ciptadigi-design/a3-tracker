...
import { addMachineComponent, adoptIntelligenceRecommendation, clearMachineComponentExclusion, effectiveProfiles, loadComponentFoundation, removeMachineComponent, saveComponent, saveProfile, setComponentStatus, setProfileStatus, syncMachineComponents } from '../services/supabase/components.js'
...
function MachineComponentsPanel({ branchName, machines, lifecycles, exclusions, profiles, components, replacementHistory, selectedMachine, onMachineChange, canManage, canInitialize, canReplace, onAdd, onSync, onRemove, onClearExclusion, onInitialize, onReplace, density, busy }) {
  ...
  return <>
    ...
    {!selectedMachine ? <div className="component-empty"><strong>No active machines in {branchName ?? 'this branch'}.</strong><span>Machine Components follows the global Branch. Model Profiles and Component Catalog remain account-wide.</span></div> : !rows.length && <div className="component-empty"><strong>No components configured for this machine model.</strong><span>No lifecycle or inventory movement was fabricated.</span>{canManage && <div><button className="secondary-button" type="button" onClick={onAdd}><Plus size={15} />Add Machine-Specific Component</button><button className="secondary-button" type="button" onClick={onSync}><RefreshCcw size={15} />Sync Model Profile</button></div>}</div>}
