...
function MachineComponentsPanel({ branchName, timezone, machines, lifecycles, exclusions, profiles, components, replacementHistory, selectedMachine, onMachineChange, canManage, canInitialize, canReplace, onRemove, onClearExclusion, onInitialize, onReplace, onReconcile, density, busy }) {
  ...
  return <>
    ...
    <div className="lifecycle-grid">{rows.map((row) => {
      ...
      const baselineKnown = row.lifecycle_status === 'baseline_known'
      const uninitialized = unknown || baselineKnown
      ...
      const capabilities = { ...machineComponentCapabilities({ assignment: row, canManage }), canInitialize: Boolean(canInitialize && uninitialized && machineComponentCapabilities({ assignment: row, canManage: true }).canInitialize) }
      return <article className={`lifecycle-card ${density === 'compact' ? 'lifecycle-card-compact' : ''} ${unknown ? 'lifecycle-unknown' : baselineKnown ? 'lifecycle-baseline-known' : `health-${row.health_status}`}`} key={row.assignment_id}>
        {density === 'compact' ? <>
        <header><h3><ComponentChannelMarker slotCode={row.slot_code} name={row.component_name} />{row.component_name}<code>{row.slot_code}</code></h3><span className="source-pill">{row.source_type === 'machine_specific' ? 'Machine-specific' : 'Inherited'}</span><span className={`health-badge ${unknown ? 'health-unknown' : baselineKnown ? 'health-baseline-known' : `health-${row.health_status}`}`}>{unknown ? 'Unknown' : baselineKnown ? 'Baseline known' : row.health_status}</span></header>
          {unknown ? <div className="compact-unknown-state"><span>Lifecycle not initialized</span>{canManage && row.reconciliation_candidate?.eligible && <button type="button" onClick={() => onReconcile(row)} disabled={busy}>Reconcile with Model Profile</button>}{capabilities.canInitialize && lifecycleAction === 'initialize' && <button type="button" onClick={() => onInitialize(row.assignment_id)} aria-label={`Initialize ${row.component_name} lifecycle`} title="Start tracking the component already installed">Initialize</button>}{capabilities.canRemove && <button type="button" onClick={() => onRemove(row)} aria-label={`Remove ${row.component_name} from machine`}>Remove from Machine</button>}</div>
          : baselineKnown ? <div className="compact-unknown-state" title="Installation date unknown"><span>Historical baseline{row.baseline_installed_counter != null ? ` · counter ${number(row.baseline_installed_counter)}` : ''} · install date unknown</span>{capabilities.canInitialize && lifecycleAction === 'initialize' && <button type="button" onClick={() => onInitialize(row.assignment_id)} aria-label={`Initialize ${row.component_name} lifecycle`} title="Confirm the actual installation date to start live tracking">Initialize</button>}{capabilities.canRemove && <button type="button" onClick={() => onRemove(row)} aria-label={`Remove ${row.component_name} from machine`}>Remove from Machine</button>}</div>
          : <div className="compact-life-row"><div className="lifecycle-progress" role="progressbar" aria-label={`${row.component_name} remaining life: ${row.remaining_percent == null ? 'unknown' : `${Number(row.remaining_percent).toFixed(1)} percent`}, ${row.health_status}`} aria-valuemin="0" aria-valuemax="100" aria-valuenow={visualPercent}><span style={{ width: `${visualPercent}%` }} /></div><strong>{row.remaining_percent == null ? 'Unknown' : `${Number(row.remaining_percent).toFixed(1)}%`}</strong>{lifecycleAction === 'replace' && <button className="compact-replace-button" type="button" onClick={() => onReplace(row.lifecycle_id)} aria-label={`Replace ${row.component_name}`}><RefreshCcw size={13} />Replace</button>}</div>}
        </> : <>
        <header><div><span className="component-icon"><Activity size={18} /></span><div><h3><ComponentChannelMarker slotCode={row.slot_code} name={row.component_name} />{row.component_name}<code>{row.slot_code}</code></h3><span>{row.source_type === 'machine_specific' ? 'Machine-specific assignment' : consumption ? 'Yield-based tracking' : 'Counter-based lifecycle'}</span></div></div>{unknown && <span className="health-badge health-unknown">Unknown</span>}{baselineKnown && <span className="health-badge health-baseline-known" title="Installation date unknown">Baseline known</span>}{!uninitialized && <span className={`health-badge health-${row.health_status}`}>{row.health_status}</span>}</header>
        {unknown ? <div className="unknown-lifecycle-body"><div><span>Installation history</span><strong>Unknown</strong></div><div><span>Tracking status</span><strong>Not initialized</strong></div><div><span>Expected baseline</span><strong>{number(row.current_profile_baseline)} clicks</strong></div><p>Start tracking a component that is already installed. Inventory will not change.</p><div className="row-actions">{canManage && row.reconciliation_candidate?.eligible && <button className="secondary-button" onClick={() => onReconcile(row)}>Reconcile with Model Profile</button>}{capabilities.canInitialize && lifecycleAction === 'initialize' && <button className="primary-button initialize-lifecycle-button" onClick={() => onInitialize(row.assignment_id)} aria-label={`Initialize ${row.component_name} lifecycle`}><span className="initialize-label-full">Initialize Lifecycle</span><span className="initialize-label-compact" aria-hidden="true">Initialize</span></button>}{capabilities.canRemove && <button className="secondary-button" onClick={() => onRemove(row)} aria-label={`Remove ${row.component_name} from machine`}>Remove from Machine</button>}</div></div>
        : baselineKnown ? <div className="unknown-lifecycle-body"><div><span>Installation history</span><strong>Historical baseline available</strong></div><div><span>Tracking status</span><strong title="Installation date unknown">Baseline known · date unknown</strong></div><div><span>Baseline counter (provenance)</span><strong>{row.baseline_installed_counter != null ? `${number(row.baseline_installed_counter)} clicks` : '—'}</strong></div><p>A prior replacement chain gives a known installed counter for this component, but no factual installation date exists — it is not being tracked as active and no health estimate is shown. Confirm the actual install date to start live tracking.</p><div className="row-actions">{canManage && row.reconciliation_candidate?.eligible && <button className="secondary-button" onClick={() => onReconcile(row)}>Reconcile with Model Profile</button>}{capabilities.canInitialize && lifecycleAction === 'initialize' && <button className="primary-button initialize-lifecycle-button" onClick={() => onInitialize(row.assignment_id)} aria-label={`Initialize ${row.component_name} lifecycle`}><span className="initialize-label-full">Initialize Lifecycle</span><span className="initialize-label-compact" aria-hidden="true">Initialize</span></button>}{capabilities.canRemove && <button className="secondary-button" onClick={() => onRemove(row)} aria-label={`Remove ${row.component_name} from machine`}>Remove from Machine</button>}</div></div>
        : <>
          <div className="lifecycle-metrics"><div><span>{consumption ? 'Used Yield' : 'Current Usage'}</span><strong>{number(row.current_usage)}</strong></div><div><span>{consumption ? 'Expected Yield' : 'Expected Life'}</span><strong>{number(row.effective_expected)}</strong></div><div><span>{consumption ? 'Estimated Remaining' : 'Remaining Clicks'}</span><strong>{number(row.remaining_clicks)}</strong></div><div><span>Remaining</span><strong>{row.remaining_percent == null ? 'Unknown' : `${Number(row.remaining_percent).toFixed(1)}%`}</strong></div><div className="replace-counter"><span>Estimated Replacement Counter</span><strong>{number(row.estimated_replacement_counter)}</strong></div></div>
          <div className="lifecycle-progress" aria-label={row.remaining_percent == null ? 'remaining life unknown' : `${Math.max(0, Number(row.remaining_percent)).toFixed(1)} percent remaining`}><span style={{ width: `${visualPercent}%` }} /></div>
    ...
    {selectedMachine && density === 'detailed' && <ReplacementHistorySummaryCard history={replacementHistory} machine={selectedMachine} timezone={timezone} onViewHistory={() => setHistoryOpen(true)} />}
    {historyOpen && selectedMachine && <ReplacementHistoryDialog history={replacementHistory} machine={selectedMachine} timezone={timezone} onClose={() => setHistoryOpen(false)} />}
  </>
}
...
export function ComponentsPage() {
  ...
  const canManage = can('components.configure')
  const canManageCatalog = (record) => record?.account_id ? can('catalog.manage') : can('catalog.global.manage')
  ...
  const selectedMachineTimezone = selectedMachine?.timezone || branch?.timezone || account.default_timezone || 'UTC'
  ...
  return <div className="page-stack components-page">
    ...
    {!canManage && <div className="permission-banner"><ShieldCheck size={18} /><span>Available component actions reflect your current workspace permissions.</span></div>}
    ...
    <section className={`component-shell component-density-${density} glass-surface`}>
      ...
      {view.tab === 'machine' && <MachineComponentsPanel branchName={branch?.name} timezone={selectedMachineTimezone} machines={scopedOperational.machines} lifecycles={scopedOperational.lifecycles} exclusions={scopedOperational.exclusions} profiles={data.profiles} components={data.components} replacementHistory={scopedOperational.replacementHistory} selectedMachine={selectedMachine} onMachineChange={(machineId) => setView((current) => ({ ...current, machineId }))} canManage={canManage} canInitialize={canInitialize} canReplace={canReplace} onRemove={(assignment) => setConfirm({ kind: 'machine-component', item: assignment })} onClearExclusion={clearExclusion} onInitialize={(assignmentId) => open('lifecycle-initialize', assignmentId)} onReplace={(lifecycleId) => open('lifecycle-replace', lifecycleId)} onReconcile={setReconcile} density={density} busy={busy} />}
      ...
      {view.tab === 'profiles' && <>
        ...
        <div className={`component-list ${density === 'compact' ? 'compact-profile-grid' : ''}`}>{visibleProfiles.map((profile) => {
          ...
          const activeActions = canManageCatalog(profile) && profile.is_active && <><button onClick={() => open('profile-edit', profile.id)} aria-label={`Edit ${profile.components?.name}`} disabled={busy}><Edit3 size={16} /></button><button onClick={() => setConfirm({ kind: 'profile', item: profile })} aria-label={`Archive ${profile.components?.name} ${profile.slot_code}`} disabled={busy}><Archive size={16} /></button></>
          const restoreAction = canManageCatalog(profile) && !profile.is_active && <button className="secondary-button compact-action" type="button" onClick={() => restoreProfile(profile)} aria-label={`Restore ${profile.components?.name} ${profile.slot_code}`} disabled={busy}><RotateCcw size={14} />Restore</button>
          return <article className={density === 'compact' ? 'compact-profile-item' : 'component-row'} key={profile.id}>
            {density === 'compact' ? <>
              ...
              <div className="adaptive-intelligence-cell"><span className={profile.adaptive_enabled ? 'adaptive-on' : 'adaptive-off'}>{profile.is_active ? (profile.adaptive_enabled ? (intelligence ? (intelligence?.confidence_label === 'no_data' ? 'No Data' : `${intelligence?.confidence_label ?? '—'} · ${intelligence?.usable_samples ?? 0}`) : 'Not yet available') : 'Disabled') : 'Archived'}</span>{intelligence && profile.is_active && <button type="button" onClick={() => open('intelligence-view', profile.id)}><BarChart3 size={13} />View Intelligence</button>}</div>
