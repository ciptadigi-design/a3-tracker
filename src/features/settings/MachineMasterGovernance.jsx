...
export function MachineMasterGovernance({ view, onViewChange, manufacturers, models, onCreate, onEdit, onSetStatus }) {
  ...
  return <section className="settings-card glass-surface machine-master-governance">
    ...
    <div className="machine-master-content">
      ...
      const manufacturer = view === 'models' ? record.manufacturer : null
      ...
      return <article key={record.id} className={!record.is_active ? 'archived' : ''}>
        ...
        <div className="machine-master-identity"><strong>{record.name}</strong><span>{view === 'manufacturers' ? record.code : manufacturer?.name ?? 'Unknown manufacturer'}</span><small>{view === 'manufacturers' ? (shared ? 'Platform shared master' : 'Account manufacturer') : `${record.model_code} · ${shared ? 'Platform shared master' : 'Account machine model'}`}</small></div>
        ...
        <div className="settings-row-actions machine-master-actions"><button type="button" aria-label={`Edit ${record.name}`} title={`Edit ${record.name}`} onClick={() => onEdit(kind, record)}><Edit3 size={15} /></button><button type="button" aria-label={`${record.is_active ? 'Archive' : 'Restore'} ${record.name}`} title={`${record.is_active ? 'Archive' : 'Restore'} ${record.name}`} onClick={() => onSetStatus(kind, record, !record.is_active)}>{record.is_active ? <Archive size={15} /> : <RotateCcw size={15} />}</button></div>
      </article>
      })}</div>
