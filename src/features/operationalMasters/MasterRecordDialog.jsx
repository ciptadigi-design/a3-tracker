...
function initialValues(kind, record) {
  if (kind === 'person') return { name: record?.name ?? '', code: record?.code ?? '', notes: record?.notes ?? '', isActive: record?.is_active ?? true, branchIds: record?.operational_person_branches?.filter((item) => item.is_active).map((item) => item.branch_id) ?? [], counterBranchIds: record?.operational_person_branches?.filter((item) => item.is_active && item.can_record_counter).map((item) => item.branch_id) ?? [], clientRequestId: crypto.randomUUID() }
  if (kind === 'manufacturer') return { name: record?.name ?? '', code: record?.code ?? '', notes: record?.notes ?? '', isActive: record?.is_active ?? true }
  return {
...
function isDraft(value) {
  return value && typeof value.name === 'string' && typeof value.notes === 'string' && typeof value.isActive === 'boolean' && (!('branchIds' in value) || (Array.isArray(value.branchIds) && Array.isArray(value.counterBranchIds)))
}
...
export function MasterRecordDialog({ kind, record, account, branches = [], manufacturers = [], onClose, onSave }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      const duplicate = ['23505', 'MANUFACTURER_ALREADY_EXISTS', 'MACHINE_MODEL_ALREADY_EXISTS'].includes(saveError.code) ? saveError.message : null
  ...
  return <BlockingDialog className="machine-dialog master-dialog glass-surface" backdropClassName="machine-dialog-backdrop" labelledBy="master-dialog-title" onClose={onClose} busy={saving}>
    <header className="dialog-header"><div className="dialog-heading"><span className="dialog-icon"><Settings2 size={22} /></span><div><span className="card-kicker">{kind === 'person' ? 'Operational master' : 'Machine master'}</span><h2 id="master-dialog-title">{title}</h2><p>{kind === 'person' ? 'Workspace records remain account-scoped and auditable.' : 'Machine masters are reusable across every Branch and governed by Platform Superusers.'}</p></div></div><button className="icon-button" type="button" onClick={onClose} disabled={saving} aria-label="Close form"><X size={19} /></button></header>
    <form className="machine-form" onSubmit={submit} noValidate><div className="machine-form-body"><div className="form-grid">
      ...
      {kind === 'person' && <fieldset className="form-field form-field-wide branch-checklist"><legend>Assigned Branches <b className="required-mark">*</b></legend>{branches.filter((branch) => branch.is_active).map((branch) => { const assigned = value.branchIds.includes(branch.id); return <div key={branch.id}><label><input type="checkbox" checked={assigned} onChange={() => change('branchIds', assigned ? value.branchIds.filter((id) => id !== branch.id) : [...value.branchIds, branch.id])} /><span>{branch.name}</span></label><label><input type="checkbox" aria-label={`Can record Daily Counter in ${branch.name}`} disabled={!assigned} checked={assigned && value.counterBranchIds.includes(branch.id)} onChange={() => change('counterBranchIds', value.counterBranchIds.includes(branch.id) ? value.counterBranchIds.filter((id) => id !== branch.id) : [...value.counterBranchIds, branch.id])} /><span>Can record Daily Counter</span></label><small>Allows this person to be selected as the operator for Daily Counter entries in this branch.</small></div> })}</fieldset>}
      {kind === 'manufacturer' && <label className="form-field"><span>Code <b className="required-mark">*</b></span><input value={value.code} onChange={(event) => change('code', event.target.value)} autoComplete="off" /></label>}
      ...
      {record && kind === 'person' && <label className="master-active-toggle"><input type="checkbox" checked={value.isActive} onChange={(event) => change('isActive', event.target.checked)} /><span><strong>Active</strong><small>Turn off to archive while preserving historical references.</small></span></label>}
