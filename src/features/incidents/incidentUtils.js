...
export function mapIncidentError(error) {
  if (`${error?.cause?.message ?? ''} ${error?.message ?? ''}`.includes('no incident changes supplied')) return 'Belum ada perubahan bidang incident untuk disimpan.'
  ...
  return 'Log error tidak dapat disimpan. Muat ulang dan coba lagi.'
}
