...
test('eligible Component without an active SKU is exposed without fabricating an item', () => {
  const result = discoverPurchaseItems(items, components, 'Drum Cyan')
  assert.deepEqual(result.itemResults, [])
  assert.deepEqual(result.missingComponents.map((component) => component.id), ['drum-c'])
})
