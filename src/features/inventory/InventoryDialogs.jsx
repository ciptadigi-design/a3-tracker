...
export function InventoryMovementDialog({ kind, account, item, items, locations, people, balances, onClose, onSubmit }) {
  ...
  const initial = { itemId: item?.id ?? items[0]?.id ?? '', locationId: '', sourceLocationId: '', destinationLocationId: '', quantity: '', direction: 'out', occurredAt: localDateTime(), personId: people[0]?.id ?? '', reason: '', notes: '' }
