...
export function InventoryItemDialog({ account, item, components, initialComponentId = '', draftEntityId, onClose, onSave }) {
  ...
  async function submit(event) {
    ...
    if (!value.name.trim()) return setError('Inventory item name is required.')
  ...
  return <DialogFrame icon={Boxes} kicker="Inventory master" title={`${item ? 'Edit' : 'Add'} inventory item`} description="Defines what is stocked. Quantity remains exclusively ledger-derived." titleId="inventory-item-title" busy={busy} onClose={onClose}>
    <form className="machine-form" onSubmit={submit} noValidate><div className="machine-form-body"><div className="form-grid">
      ...
      <label className="form-field"><span>SKU <small>Optional</small></span><input value={draft.value.sku} onChange={(event) => change('sku', event.target.value)} autoComplete="off" /><small className="field-hint">Optional internal/vendor stock code.</small></label>
...
export function InventoryMovementDialog({ kind, account, branchId, item, items, locations, people, balances, onClose, onSubmit }) {
  ...
  return <DialogFrame icon={config.icon} kicker={config.kicker} title={config.title} description={config.description} titleId="inventory-movement-title" busy={busy} onClose={onClose}>
    ...
    </div><footer className="dialog-actions form-action-footer"><button className="draft-reset-button" type="button" onClick={() => draft.resetDraft(initial)} disabled={!draft.hasDraft || busy} aria-label="Reset draft" title="Reset draft"><RotateCcw size={15} />Reset draft</button><button className="secondary-button" type="button" onClick={onClose} disabled={busy}>Cancel</button><button className={kind === 'adjustment' && draft.value.direction === 'out' ? 'danger-button' : 'primary-button'} type="submit" disabled={busy}>{busy && <LoaderCircle className="spin" size={17} />}{busy ? 'Posting…' : config.submit}</button></footer></form>
  </DialogFrame>
}
