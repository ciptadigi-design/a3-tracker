  ...
  return <BlockingDialog className="machine-dialog component-dialog lifecycle-dialog glass-surface" backdropClassName="machine-dialog-backdrop" labelledBy="lifecycle-dialog-title" onClose={onClose} busy={saving}>
    ...
    <form className="machine-form" onSubmit={submit}>
      <div className="machine-form-body">
        ...
        <div className="lifecycle-context"><div><span>Component</span><strong>{lifecycle.component_name}</strong></div><div><span>Machine</span><strong>{machine.machine_code}</strong></div><div><span>Assignment</span><strong>{lifecycle.source_type === 'machine_specific' ? 'Machine-specific' : 'Inherited'}</strong></div><div><span>Current recorded counter</span><strong>{formatCounter(lifecycle.latest_effective_counter)}</strong></div></div>
