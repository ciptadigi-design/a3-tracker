...
export function ReplacementHistorySummaryCard({ history, machine, timezone, onViewHistory }) {
  ...
  return <section className="replacement-history-section replacement-history-summary" aria-labelledby="replacement-history-title">
    ...
    {rows.length
      ? <div className="replacement-history-summary-body">
        {latest && <p className="replacement-history-latest"><strong>{latest.component_name}</strong> · {dateTime(latest.replaced_at, timezone)} · {replacementReasonLabels[latest.replacement_reason] ?? 'Reason not recorded'}</p>}
...
export function ReplacementHistoryDialog({ history, machine, timezone, onClose }) {
  const rows = historyForMachine(history, machine?.id)
  ...
  return <BlockingDialog className="machine-dialog replacement-history-dialog glass-surface" backdropClassName="machine-dialog-backdrop" labelledBy="replacement-history-dialog-title" onClose={onClose}>
    ...
    <div className="machine-form-body replacement-history-dialog-body">
      ...
      {filtered.length
        ? <div className="replacement-history-list">{visible.map((event) => <ReplacementHistoryCard event={event} timezone={timezone} key={event.replacement_event_id} />)}</div>
