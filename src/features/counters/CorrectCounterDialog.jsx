...
export function CorrectCounterDialog({ reading, onClose, onCorrected }) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !isSubmitting) onClose() }}>
      <section className="confirm-dialog correction-dialog glass-surface" role="dialog" aria-modal="true" aria-labelledby="correction-dialog-title">
        <header className="dialog-header"><span className="dialog-icon"><FilePenLine size={22} /></span><button className="icon-button" type="button" onClick={onClose} disabled={isSubmitting} aria-label="Close correction form"><X size={19} /></button></header>
        <h2 id="correction-dialog-title">Correct latest reading</h2>
        <p>History is append-only. Replace the latest value with an audited correction, or void it without deleting the record.</p>
        <form className="correction-form" onSubmit={handleSubmit}>
          <div className="correction-action-tabs"><button type="button" className={action === 'replace' ? 'selected' : ''} onClick={() => { setAction('replace'); setError(null) }}>Replace value</button><button type="button" className={action === 'void' ? 'selected danger-tab' : ''} onClick={() => { setAction('void'); setError(null) }}>Void reading</button></div>
          ...
          <label className="form-field"><span>Correction reason <b className="required-mark">*</b></span><textarea value={reason} onChange={(event) => { setReason(event.target.value); setError(null) }} rows="3" placeholder="Why is the original reading incorrect?" /></label>
