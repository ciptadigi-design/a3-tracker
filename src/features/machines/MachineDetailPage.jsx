...
export function MachineDetailPage({ machineId, navigate }) {
  const { account, branches, membership, setSelectedBranchId } = useTenant()
  ...
  const actions = canManage && machine.is_active ? <div className="detail-actions"><button className="secondary-button" type="button" onClick={() => setShowEdit(true)}><Edit3 size={17} /> Edit machine</button><button className="danger-outline-button" type="button" onClick={() => setShowRetire(true)}>Retire machine</button></div> : null
  ...
  return (
    <div className="page-stack machine-detail-page">
      ...
      {showEdit && <MachineFormDialog mode="edit" machine={machine} account={account} branches={branches} branchId={branch?.id} onClose={() => setShowEdit(false)} onSave={handleUpdate} />}
