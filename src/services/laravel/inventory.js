...
export async function receiveInventoryPurchase({ purchaseId, values, clientRequestId }) { return unwrapData(await apiClient.post(`/purchases/${purchaseId}/receive`, { location_id: values.locationId, received_at: values.receivedAt, person_id: values.personId || null, notes: values.notes || null, client_request_id: clientRequestId, lines: values.lines.filter((line) => Number(line.quantity) > 0).map((line) => ({ purchase_line_id: line.purchaseLineId, quantity: line.quantity })) })) }
