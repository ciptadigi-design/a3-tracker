...
The drawer remains scrollable with safe-area padding. Static responsive contracts cover desktop duplication removal, TopBar preservation, mobile collapsed/expanded disclosure, canonical My Account navigation, canonical logout abstraction, and the intermediate tablet single-surface rule.

## Browser-backed Laravel runtime evidence (M2.12A.3)

Evidence was captured against a disposable local MySQL database (`a3_tracker_browser_smoke`, Homebrew MySQL on `127.0.0.1:3306`) with `APP_ENV=production`, `APP_DEBUG=false`, `VITE_DATA_BACKEND=laravel`, and no `VITE_SUPABASE_*` variables. Laravel ran from `public/index.php` on `127.0.0.1:8000`; Vite served the production frontend on `127.0.0.1:5173` with a local `/api` and `/sanctum` proxy. A disposable Platform Superuser fixture authenticated by username in headless Google Chrome using the Chrome DevTools Protocol.

`/api/v1/health` and `/api/v1/version` returned healthy Laravel/MySQL responses. Direct browser navigation/refresh passed for `/`, `/my-account`, `/machines`, `/daily`, `/components`, `/inventory`, `/machine-cost`, `/errors`, `/reports`, and `/settings`; each rendered without an uncaught exception or missing-adapter error. The browser observed 138 local Laravel API/Sanctum requests and **0 Supabase operational requests**. A first pass exposed an incident adapter envelope mismatch; `services/laravel/incidents.js` was corrected to preserve the `{ incidents }` and `{ incident }` envelopes consumed by the UI, and the rerun passed.

Responsive capture passed at 390×844: hamburger drawer opened, the compact identity row was collapsed by default, actions expanded inline on tap, no horizontal overflow occurred, and My Account navigated to `/my-account` while closing the drawer. Logout ended the disposable Laravel session and returned the login view with no Supabase traffic. At 768px exactly one drawer account surface was discoverable; at 1440px the TopBar profile dropdown was present and the sidebar account footer was absent. Browser console review after the fix found no critical runtime errors (Vite HMR informational messages only). Screenshots remain temporary evidence at `/tmp/konica-m2.12a3-mobile-expanded.png` and `/tmp/konica-m2.12a3-desktop-account.png`; they are not product assets.

This is local production-like runtime evidence, not Hostinger Apache/rewrite or live-environment proof. No hosted data, DNS, Vercel, Hostinger, or Maintenance state was changed.
